...
Raspberry Pi Debug Probe 能让向 Pico 2 烧录固件的过程更加便捷。没有探针时，每次上传新固件都需按住 BOOTSEL 按钮。探针同时提供完善的调试支持，非常实用。

该工具为可选项。除专门介绍调试探针的章节外，本书的其它内容均可在没有探针的情况下学习。我在刚开始接触 Pico 时也是先不使用探针，随后才购入。
...
如果预算有限，可以暂时不购买，因其价格大约为一块 Pico 2 的两倍；若成本不是问题，则非常值得入手。如有第二块 Pico，也可将其配置成低成本的调试探针。
